...
private:
  ...
  bool pcap;                                    //enable disable pcap eport files
...
SiftTest::Configure (int argc, char **argv)
{
  ...
  Ptr<ConstantRandomVariable> s = CreateObject<ConstantRandomVariable> ();
  CommandLine cmd;
...
/*  std::stringstream convert;
  ...
  switch (mobilityModel)
    ...
    case RANDOMDIRECTION2:
      speed = "ns3::UniformRandomVariable[Min=1.0|Max=" + convert.str () + "u]";
      convert.str ("");
      convert.clear ();
      convert << pauseTime;
    ...
    case RANDOMWAYPOINT:
      speed = "ns3::UniformRandomVariable[Min=0.3|Max=" + convert.str () + "u]";
      convert.str ("");
      convert.clear ();
      convert << pauseTime;
      pause = "ns3::ConstantRandomVariable[Constant=" + convert.str () + "s]";
...
*/
   adhocMobility.SetMobilityModel ("ns3::GaussMarkovMobilityModel",
  ...
  adhocMobility.SetPositionAllocator ("ns3::RandomBoxPositionAllocator",
...
SiftTest::CreateDevices ()
{
  ...
  switch (algo)
    {
    case SIFT:
      stream = ascii.CreateFileStream ("siftTrace2.tr");
      if (pcap)
        {
